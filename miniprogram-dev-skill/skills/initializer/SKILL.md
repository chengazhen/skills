...
6. 确认配置和登录态均有效后，再执行 `project_open_window`
...
### project_close_window — 关闭项目窗口

关闭包含模拟器的项目窗口，不等于关闭整个 DevTools。

```bash
wechatide -c <clientName> -t project_close_window --project <project>
```
...
| 用户意图 | 选择 |
|---------|------|
| "先把项目打开" / "接管当前项目" | 先读取 `project.config.json` 确认 appid 有效 → `project_open_window` |
| "看看现在登没登录" | `check_devtools_status` |
| "需要主动登录" | `scan_login` |
| "关闭项目窗口" | `project_close_window` |
| "退出 DevTools" | `quit` |
